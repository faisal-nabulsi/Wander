...
//
//  ┌─────────────────────────────────────────────────────────────────────────┐
//  │ WHY THIS IS GATED BEHIND THE `WANDER_HEALTHKIT` COMPILE FLAG (READ THIS)  │
//  ├─────────────────────────────────────────────────────────────────────────┤
//  │ HealthKit.framework is loaded by dyld at APP LAUNCH (auto-linked from     │
//  │ `import HealthKit`). iOS TERMINATES an app that links a restricted        │
//  │ framework (HealthKit) whose signature lacks the matching entitlement.     │
//  │ On the FREE-Apple-ID sideload model the `com.apple.developer.healthkit`   │
//  │ entitlement is stripped at re-sign, so a HealthKit-linked build CRASHES   │
//  │ ON OPEN for every free user (this is exactly what shipped in build 29).   │
//  │                                                                           │
//  │ Build 29 tried to guard with `#if canImport(HealthKit)` — but that is     │
//  │ ALWAYS TRUE on iOS (the framework exists in the SDK), so it linked        │
//  │ regardless and still crashed. The ONLY reliable guard is a CUSTOM flag    │
//  │ the default build never defines. Hence `WANDER_HEALTHKIT`:                │
//  │                                                                           │
//  │   • DEFAULT build (OTA / free sideload): flag UNSET → every HealthKit     │
//  │     line below is excluded → the framework is NEVER auto-linked → no      │
//  │     launch crash. The class compiles as an honest no-op (status stays     │
//  │     `.unavailable`); the Settings toggle shows "not available".           │
//  │   • HEALTHKIT build (paid / service-cert, pre-signed): build with         │
//  │     `SWIFT_ACTIVE_COMPILATION_CONDITIONS=... WANDER_HEALTHKIT` → the full  │
//  │     writer is compiled and HealthKit is linked. The re-signing profile    │
//  │     supplies the `com.apple.developer.healthkit` entitlement, so writes   │
//  │     succeed. This IPA must ONLY be given to installs signed with a cert   │
//  │     that grants HealthKit — NEVER shipped as the free-sideload OTA build. │
//  └─────────────────────────────────────────────────────────────────────────┘
//
//  The public API is identical in both modes, so the movement hooks
//  (`WalkModeView` / `RouteModeView`), the Settings toggle, and `AdventureSyncView`
//  compile and behave unchanged whether or not the flag is set.
//
...
final class AdventureSyncManager: ObservableObject {
    ...
    enum Status: Equatable {
        ...
        /// this build's signature lacks the HealthKit entitlement / the build was
        /// compiled without `WANDER_HEALTHKIT`, so the OS won't grant write access.
        /// Honest no-op with a "needs paid-signing install" hint.
        case unavailable
    ...
    /// API-parity no-op retained from the earlier stub in case any call site still
    /// references it. Re-checks authorization when HealthKit is compiled in.
    func refresh() {
        refreshAuthorizationStatus()
    }
